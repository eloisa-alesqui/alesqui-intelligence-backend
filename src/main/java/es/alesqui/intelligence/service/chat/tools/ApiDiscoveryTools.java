...
import static es.alesqui.intelligence.service.chat.tools.support.EndpointSupport.resolveEndpoint;
import static es.alesqui.intelligence.service.chat.tools.support.SseSupport.emitStatus;
import static es.alesqui.intelligence.service.chat.tools.support.ToolContextSupport.getUserId;
...
public class ApiDiscoveryTools {
    ...
    public String listApis(ToolContext toolContext) {
        emitStatus(toolContext, "Listing available APIs...");
        ...
        try {
            ...
            if (apis == null || apis.isEmpty()) {
                emitStatus(toolContext, "No APIs found.");
                return "No available APIs were found.";
            }

            emitStatus(toolContext, "Found " + apis.size() + " APIs.");
            ...
            emitStatus(toolContext, "Failed to list APIs.");
    ...
    public String listEndpoints(@ToolParam(description = "The exact name of the API to inspect.") String apiName, ToolContext toolContext) {
        emitStatus(toolContext, "Listing endpoints for API: " + apiName + "...");
        ...
        try {
            ...
            emitStatus(toolContext, "Found " + filteredEndpoints.size() + " endpoints for " + apiName + ".");
            ...
            emitStatus(toolContext, "Failed to list endpoints.");
    ...
    public String inspectEndpoint(
        ...
        emitStatus(toolContext, "Inspecting endpoint details for API: " + apiName + ", Operation: " + operationId + "...");
        ...
        try {
            ...
            UnifiedEndpoint endpoint = resolveEndpoint(api, operationId);
            ...
            try {
                String conversationId = toolContext != null && toolContext.getContext() != null
                        ? (String) toolContext.getContext().get(ToolContextKeys.CONVERSATION_ID)
                        : null;
            ...
            emitStatus(toolContext, "Endpoint details assembled.");
            return sb.toString();
