...
import es.alesqui.intelligence.model.api_spec.postman.enums.FormParameterType;
...
public class FormParameter {
	...
	/**
     * File source (for file type parameters). Can be a string, a list of strings, or null.
     */
    @Field("src")
    @JsonProperty("src")
    private Object src;
	 ...
	 *
	 * @return true if the parameter has a valid value or source, false otherwise.
	 */
	public boolean hasValue() {
	    if (isText()) {
	        ...
	        if (src instanceof String) {
	            return !((String) src).isBlank();
	        }
	        if (src instanceof List) {
	            return !((List<?>) src).isEmpty() && ((List<?>) src).stream().allMatch(item -> item instanceof String && !((String) item).isBlank());
	        }
	    }
	    return false;
	}
	    ...
	    if (isText()) {
	        ...
	        if (src instanceof List) {
	            ...
	            return srcList.stream()
	                          ...
	                          .map(Object::toString)
	                          .reduce((a, b) -> a + ", " + b)
	                          .orElse(null); // Combine list into a comma-separated string
	        }
	    }
	    return null; // If no effective value exists
	}
