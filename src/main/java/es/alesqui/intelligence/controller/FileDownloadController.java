		...
		return SecurityUtils.getCurrentUsername()
				.flatMap(username -> {
					try {
						...
						Path userFileDir = this.tempFileDir.resolve(safeUsername);

						// Resolve and normalize the file path to prevent directory traversal attacks
						Path filePath = userFileDir.resolve(filename).normalize();

						// Security validation: Ensure the resolved path is within the user's subdirectory
						if (!filePath.startsWith(userFileDir)) {
							log.warn("Path traversal attempt detected for file: {} by user: {}", filename, username);
							return writeErrorResponse(exchange.getResponse(), HttpStatus.BAD_REQUEST);
						}

						if (!Files.exists(filePath)) {
							log.debug("File not found: {} for user: {}", filename, username);
						...
						log.debug("Successfully serving file: {} for user: {}", filename, username);
