 ...
 *
 * The implementation delegates to getHistoryForUser() (which calls
 * repository.findConversationSummariesByUsername) for summaries, then calls
 * findByConversationIdOrderByTimestampAsc() per conversation.
 */
...
class ConversationServiceExportTest {
    ...
    private ConversationSummaryDTO summary(String convId, String title, Instant lastUpdated) {
        return new ConversationSummaryDTO(convId, title, lastUpdated);
    }
    ...
    class HappyPath {
        ...
        void shouldExportTwoConversations() {
            ...
            // Repository returns summaries sorted DESC by lastUpdated
            when(repository.findConversationSummariesByUsername(USERNAME))
                    .thenReturn(Flux.just(
                            summary(CONV_ID_2, "Other question", now),
                            summary(CONV_ID_1, "First question", oneHourAgo)
                    ));
        ...
        void shouldUseFirstChronologicalPromptAsTitle() {
            ...
            when(repository.findConversationSummariesByUsername(USERNAME))
                    .thenReturn(Flux.just(summary(CONV_ID_1, "Very first question", oneHourAgo)));
            when(repository.findByConversationIdOrderByTimestampAsc(CONV_ID_1))
        ...
        void shouldSetLastUpdatedToMostRecentTimestamp() {
            ...
            when(repository.findConversationSummariesByUsername(USERNAME))
                    .thenReturn(Flux.just(summary(CONV_ID_1, "Q1", oneHourAgo)));
            when(repository.findByConversationIdOrderByTimestampAsc(CONV_ID_1))
        ...
        void shouldMarkErrorRecordsInMessages() {
            ...
            when(repository.findConversationSummariesByUsername(USERNAME))
                    .thenReturn(Flux.just(summary(CONV_ID_1, "Question", now)));
            when(repository.findByConversationIdOrderByTimestampAsc(CONV_ID_1))
        ...
        void shouldSortConversationsByLastUpdatedDesc() {
            ...
            // Repository already returns in DESC order; concatMap preserves that order
            when(repository.findConversationSummariesByUsername(USERNAME))
                    .thenReturn(Flux.just(
                            summary(CONV_ID_2, "Recent question", now),
                            summary(CONV_ID_1, "Old question", twoHoursAgo)
                    ));
            when(repository.findByConversationIdOrderByTimestampAsc(CONV_ID_2))
    ...
    class OwnershipEnforcement {
        ...
        void shouldOnlyIncludeRecordsForQueriedUser() {
            ...
            when(repository.findConversationSummariesByUsername(USERNAME))
                    .thenReturn(Flux.just(summary(CONV_ID_1, "My question", oneHourAgo)));
            when(repository.findByConversationIdOrderByTimestampAsc(CONV_ID_1))
